...
import env, { ethers, upgrades, network } from 'hardhat';
const fs = require('fs');
...
async function main() {
  ...
  await upgrades.upgradeProxy(assetFactoryUpgradableAddress, assetFactoryUpgradableFatory);

  console.log('proxy contract', assetFactoryUpgradableAddress);
