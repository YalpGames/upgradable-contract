  ...
  PausableUpgradeable
{
  ...
  /**
    ...
    @param newFactory address of newFactory
   */
  function setCoinFactory(address newFactory) external virtual override onlyOwner {
    address oldFactory = CoinFactory;
    require(newFactory != address(0), 'GC: ve can not 0');
  ...
  */
  ...
  function addGauge(
    ...
    require(msg.sender == owner() || msg.sender == address(CoinFactory), 'GC: only admin or CoinFactory');
  ...
  function initialize(address votingEscrow_, address CoinFactory_) public initializer {
    require(votingEscrow_ != address(0) && CoinFactory_ != address(0), 'GC: address can not 0');
